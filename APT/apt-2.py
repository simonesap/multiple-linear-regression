...
cl = yf.Ticker('CL=F').history(start=start, end=end) #Apple Inc.
...
target = pd.DataFrame()
target['return'] = cl['Close'].pct_change(1)*100 #i dati sono ritardati di un giorno per evitare bias
...
features['gold'] = gold['Close'].pct_change(1)*100
...
dataset['crudeoil'] = cl['Close'].pct_change(1)*100
...
dataset['gold'] = gold['Close'].pct_change(1)*100
